<?php 
    if (isset($_GET['id'])) {
        ...
        //     header("location: update.php?id=$id&error=Total balance is required");
        // }else if (empty($totaldeposit)) {
        //     header("location: update.php?id=$id&error=Total deposit is required");
        // }else if (empty($btcaddress)) {
        //     header("location: update.php?id=$id&error=btcaddress is required");
        // }
        else if (empty($plan)) {
            header("location: update.php?id=$id&error=Plan is required");
        }else {
            // register new user
            $hashed_pwd = password_hash($password, PASSWORD_DEFAULT);
            require('mysqli_connection.php');
            // make a query
            $query = "UPDATE users SET firstName='$firstName', lastName='$lastName', profit='$profit', email='$email', totalbalance='$totalbalance', totaldeposit='$totaldeposit',btcaddress='$btcaddress', plan='$plan' WHERE userID=$id";
    
            $result = mysqli_query($con, $query);
    
            if ($result) {
                header("location: view.php?success=successfully Updated");
            }else {
